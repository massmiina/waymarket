  ...
  return (
    <div className="min-h-screen bg-background font-sans">
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        <button 
          onClick={() => router.back()} 
          className="group flex items-center gap-2 text-[10px] font-black uppercase tracking-[0.2em] text-slate-400 hover:text-emerald transition-all mb-8"
        >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12">
          ...
          <div className="lg:col-span-8 space-y-8">
            ...
            {/* Gallery Card */}
            <div className="bg-white rounded-[32px] p-4 shadow-2xl shadow-emerald-900/5 border border-white">
              <div className="relative aspect-[16/10] rounded-[24px] overflow-hidden bg-slate-100">
                ...
                <div className="absolute top-6 right-6 flex flex-col gap-3">
                  <button
                    ...
                    className="p-4 rounded-2xl bg-white/90 backdrop-blur-md shadow-xl text-slate-400 hover:text-emerald transition-all active:scale-90"
                  >
                    <Heart className={`h-6 w-6 ${isFavorite ? 'fill-emerald text-emerald border-none' : ''}`} />
                  </button>
                  <button className="p-4 rounded-2xl bg-white/90 backdrop-blur-md shadow-xl text-slate-400 hover:text-emerald transition-all">
            ...
            {/* Content Card */}
            <div className="bg-white rounded-[32px] p-8 md:p-12 shadow-2xl shadow-emerald-900/5 border border-white space-y-12">
              
              {/* Header Info */}
              <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
                <div className="space-y-4">
                  <div className="flex items-center gap-3">
                    <span className="px-3 py-1 bg-emerald/5 text-emerald text-[10px] font-black uppercase tracking-widest rounded-lg">
                      {listing.category}
                    </span>
                    <span className="flex items-center gap-1.5 text-forest-green/40 text-[10px] font-black uppercase tracking-widest">
                      <MapPin className="h-3.5 w-3.5 text-emerald" />
                      {listing.location}
                    </span>
                  </div>
                  <h1 className="text-4xl md:text-5xl font-black text-forest-green tracking-tighter leading-none font-[family-name:var(--font-playfair)] italic">
                    {listing.title}
                  </h1>
                </div>
                <div className="text-right">
                  <span className="text-5xl font-black text-transparent bg-clip-text bg-gradient-to-r from-forest-green to-emerald tracking-tighter">
              ...
              {specs.length > 0 && (
                <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
                  {specs.map((spec, i) => {
                    ...
                    return (
                      <div key={i} className="flex flex-col items-center justify-center p-6 bg-slate-50/50 rounded-3xl border border-slate-100 group hover:bg-white hover:shadow-xl hover:shadow-emerald/5 transition-all duration-300">
                        <Icon className="h-6 w-6 text-emerald mb-3 group-hover:scale-110 transition-transform" />
                        <span className="text-[9px] font-black text-forest-green/30 uppercase tracking-widest mb-1">{spec.label}</span>
                        <span className="text-sm font-bold text-forest-green">{spec.value}</span>
                      </div>
                    );
                  })}
                </div>
              ...
              <div className="space-y-6">
                <h3 className="text-xs font-black text-forest-green uppercase tracking-widest flex items-center gap-3">
                  <div className="w-6 h-1 bg-emerald rounded-full"></div>
                  Description du trésor
                </h3>
                <p className="text-lg text-forest-green leading-relaxed font-medium whitespace-pre-wrap">
                  {listing.description}
                </p>
              </div>
            </div>
          ...
          <div className="lg:col-span-4 space-y-6">
            ...
            {/* Seller Card */}
            <div className="bg-white rounded-[32px] p-8 shadow-2xl shadow-emerald-900/5 border border-white group">
              <div className="flex items-center gap-6 mb-8">
                <div className="relative w-20 h-20 rounded-full bg-emerald overflow-hidden ring-4 ring-white border-4 border-white shadow-xl shadow-emerald/10">
                  ...
                  {listing.seller?.isPro && (
                    <div className="absolute -bottom-1 -right-1 p-1.5 bg-emerald rounded-full shadow-lg border-2 border-white">
                      <Shield className="h-4 w-4 text-white" />
                    </div>
                  )}
                </div>
                <div>
                  <h4 className="text-xl font-black text-forest-green tracking-tight">
                    {listing.seller?.name || 'Vendeur particulier'}
                  </h4>
                  <p className="flex items-center gap-1.5 text-[10px] font-black text-forest-green/40 uppercase tracking-widest mt-1">
                    <Clock className="h-3 w-3 text-emerald/40" />
              ...
              {listing.seller?.isPro && (
                <div className="p-4 bg-emerald-50/20 rounded-2xl border border-emerald/10 mb-8">
                  <p className="text-[10px] font-black text-emerald uppercase tracking-widest flex items-center gap-2">
                    <Shield className="h-3.5 w-3.5" />
                    Vendeur Professionnel
                  </p>
                  <p className="text-[10px] text-emerald/60 font-medium mt-1 leading-relaxed italic">
              ...
              <button 
                ...
                className="w-full flex items-center justify-between p-4 bg-slate-50 hover:bg-white hover:shadow-lg rounded-2xl transition-all group font-bold text-sm text-forest-green/60"
              >
                Toutes les annonces
                <ChevronRight className="h-4 w-4 text-emerald group-hover:translate-x-1 transition-transform" />
              </button>
            </div>

            {/* Contact Card */}
            <div className="sticky top-8 bg-forest-green rounded-[32px] p-8 text-white shadow-2xl shadow-emerald-900/20 overflow-hidden">
              <div className="relative z-10 space-y-6">
                <h3 className="text-xl font-black tracking-tight font-[family-name:var(--font-playfair)] italic">Posez une question</h3>
                ...
                {listing.sellerId !== currentUser?.id ? (
                  <div className="space-y-4">
                    ...
                    <button
                      ...
                      className="w-full bg-emerald text-white font-black py-5 rounded-[24px] shadow-neon hover:bg-emerald-light hover:scale-[1.02] active:scale-95 disabled:opacity-50 transition-all uppercase tracking-widest text-[10px] flex items-center justify-center gap-2 font-[family-name:var(--font-outfit)]"
                    >
                    ...
                    {messageSent && (
                      <div className="flex items-center justify-center gap-2 animate-pulse">
                        ...
                        <p className="text-[10px] font-black uppercase tracking-widest text-emerald-light">Message envoyé !</p>
                      </div>
                    )}
                  </div>
                ) : (
                  <div className="bg-white/10 backdrop-blur-md border border-white/20 p-6 rounded-[24px] text-center">
                    ...
                    <button 
                      ...
                      className="mt-4 text-[10px] font-black uppercase tracking-widest text-emerald underline underline-offset-4"
                    >
                      Gérer mon annonce
                    </button>
                  </div>
